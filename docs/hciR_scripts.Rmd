...
This guide describes how to run alignments and quality metrics
on the [CHPC] clusters and then run [multiQC] and other `R` scripts on the output files.
The scripts are located in `/home/BioApps/hciR`  on `uinta`, so add that directory
to your path to run the `*.R` files below.  If you have Fastq files starting with `14176X`, the basic
steps to align 50 bp single-end reads to mouse are listed below.

```{bash fdt, eval=FALSE}
setup_jobs.R -d mouse -i <fastq directory> -e <my hci email>
pstart 14176X*
module load multiqc
multiqc .
...
Run `setup_jobs.R` to create a `cmd.txt` file and link the Fastq files in the input
directory to create 12 separate sample directories (14176X1 to 14176X12).


```{bash setup, eval=FALSE}
setup_jobs.R -d mouse -i Fastq -e chris.stubben
...
The default is to align 50 bp single end reads to the Ensembl 94 human reference.
The `cmd.txt` file can also be configured to align paired end or the new Qiagen small RNA libraries with UMI
tags,  with references from human, mouse, fly, pig, rabbit, elephant, vervet, rat, sheep, worm,
...
Add a markdown file with code to run DESeq2 using all possible contrasts.  This script
requires a sample table as input with the first column matching the count column names and
treatment column with sample groups. Render the markdown file to save an Excel file and
 html report with sample visualizations.
